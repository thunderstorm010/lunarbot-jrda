...
class komutttahuojrwekdrs: ListenerAdapter() {
    val kayit_allowed_channel = 738553650611224590L
    val ip_allowed_channel = 738553651655737377L
    override fun onGuildMessageReceived(event: GuildMessageReceivedEvent) {
        if (event.author.isBot) return
        if (event.message.contentRaw == "!ip" && event.channel.idLong != ip_allowed_channel) {
            EmbedBuilder()
                    .setTitle("Özel Komut")
                    .setDescription("Bu komut bu kanalda izin verilmemektedir. Lütfen " + event.jda.getGuildChannelById(ip_allowed_channel)!!.name + " kanalına gidiniz.")
                    .setAuthor(
        ...
        if (event.message.contentRaw == "!kayıt" && event.channel.idLong != kayit_allowed_channel) {
            EmbedBuilder()
                    .setTitle("Özel Komut")
                    .setDescription("Bu komut bu kanalda izin verilmemektedir. Lütfen " + event.jda.getGuildChannelById(kayit_allowed_channel)!!.name + " kanalına gidiniz.")
                    .setAuthor(
                            event.author.name + "#" + event.author.discriminator + ", ",
                            null,
                            event.author.effectiveAvatarUrl
        ...
        when(event.message.contentRaw) {
            "!ip" -> {
                EmbedBuilder()
                        ...
                        .setTitle("!ip")
                        .setDescription("**:star2: Sunucu IP Adresi : 185.193.165.2**\n" +
            ...
            "!kayıt" -> {
                EmbedBuilder()
                        ...
                        .setTitle("!kayıt")
                        .setDescription( "``\uD83C\uDF1FSunucumuza kayıt olmak için aşağıdaki formu doldurmalısınız \uD83C\uDF1F``\n" +
                                "\n" +
                                "```Nicknameniz : \n" +
                                "İsminiz : \n" +
                                "Yaşınız : \n" +
